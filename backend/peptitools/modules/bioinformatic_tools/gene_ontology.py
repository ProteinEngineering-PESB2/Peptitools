...
class GeneOntology:
    ...
    def find_and_load_data(self):
        ...
        results = []
        params = [
            (".MFO.txt", "molecular_function"),
            (".BPO.txt", "biological_process"),
            (".CCO.txt", "celular_component"),
        ]
        for param_tuple in params:
            try:
                df_go = pd.read_csv(self.output_path + param_tuple[0], header=None, sep="\t")
                df_go.rename(
                ...
                for single_go in unique_go:
                    ...
                    go_array.append({"id_seq": str(single_go), "results": temp.to_dict("records")})
